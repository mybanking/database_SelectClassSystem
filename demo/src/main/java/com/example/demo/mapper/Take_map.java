...
public interface Take_map {
    ...
    @Select("select * from take where year=#{year} and week=#{week}")
    List<Take> search_classroom1(@Param("year") String year,@Param("week")String week);

    @Select("select * from take where classroomId in (select classroomId from classroom where buildingName= #{buildingName}) and year = #{year} and week = #{week}")
    List<Take> search_classroom2(@Param("buildingName")String buildingName,@Param("week")String week,@Param("year")String year);

    @Insert("insert into take(takeId,classroomId,takeDate,startTime,endTime,type,information) values(#{takeId},#{classroomId},#{takeDate},#{startTime},#{endTime},#{type},#{information})")
